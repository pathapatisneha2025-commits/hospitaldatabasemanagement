...
router.get("/leavessummary/:employee_id", async (req, res) => {
  try {
    ...
    // 1️⃣ Fetch total working days (no month filter)
    ...
    // 2️⃣ Monthly paid leaves = days in current month - working_days
    const daysInCurrentMonth = getDaysInMonth(currentMonth, currentYear);
    const monthlyPaidLeaves = Math.max(daysInCurrentMonth - totalDays, 0);

    // 3️⃣ Annual paid leaves = assume 12 months * days in month - total working days
    const totalAnnualDays = totalDays * 12; // crude estimation
    const totalDaysInYear = Array.from({ length: 12 }, (_, i) => getDaysInMonth(i + 1, currentYear))
                                 .reduce((a, b) => a + b, 0);
    const totalAnnualPaidLeaves = Math.max(totalDaysInYear - totalAnnualDays, 0);

    // 4️⃣ Calculate used leaves for the current month
    ...
    // 5️⃣ Calculate used leaves for the year
