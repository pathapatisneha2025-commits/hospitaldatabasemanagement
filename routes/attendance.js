...
router.post("/verifyface", upload.single("image"), async (req, res) => {
  try {
...
router.post("/verify-face", upload.single("image"), async (req, res) => {
  try {
    ...
    if (!capturedUrl) {
      return res.status(400).json({
        success: false,
        message: "Captured image URL missing",
      });
    }
...
const OFFICE_LAT = 17.677572;
const OFFICE_LNG = 83.199835;
...
router.get("/export", async (req, res) => {
  try {
    ...
    if (date) {
      params.push(date);
      dateFilter = `WHERE DATE(a.timestamp) = $1`;
    }
...
router.get("/employee-history", async (req, res) => {
  try {
    ...
    if (!employee_id && !phone) {
      return res.status(400).json({
        success: false,
        message: "employee_id or phone is required",
      });
    }
